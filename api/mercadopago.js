...
export default async function handler(req, res) {
  ...
  // El método del usuario asienta las ventas marcándolas COBRADAS en la caja del procesador (incluido el
  // pendiente) → la apertura y cada cierre ya incluyen lo "a liquidar". Por eso el acreditado cuenta SOLO las
  // ventas del mes en curso (date_approved >= 1° del mes): las de meses previos que liberan ahora ya están en
  // la caja (evita doble conteo). Las previas van a acreditado_mes_anterior (informativo, no suma).
  ...
  try {
    ...
    let acreditadoMesAnterior = 0;   // ventas previas liberadas este mes (ya en la caja/apertura, informativo)
    ...
    for (let page = 0; page < MAX_PAGES && offset < total; page++) {
      ...
      for (const p of rows) {
        ...
        if (rel && rel > nowMs) {
          ...
          acreditadoMesAnterior += neto;   // venta previa liberada este mes → ya está en la caja, NO sumar
        } else {
          acreditado += neto;   // vendido y liberado este mes = plata nueva que aún no está asentada en Numbers
        }
      }
      offset += LIMIT;
    ...
    res.end(JSON.stringify({
      ...
      acreditado_mes_anterior: Math.round(acreditadoMesAnterior),  // ventas previas liberadas este mes (ya en la caja)
      moneda,
      proximos,
      count,
