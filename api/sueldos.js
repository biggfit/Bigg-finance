// api/sueldos.js — Vercel Serverless Function
//
// Proxea /api/sueldos hacia Google Apps Script siguiendo los redirects
...
//   VITE_SUELDOS_API_URL  →  URL del Apps Script de BIGG Sueldos (exec)
//   VITE_SHEETS_TOKEN     →  token secreto compartido

import { request as httpsRequest } from 'https';

const SUELDOS_URL = process.env.VITE_SUELDOS_API_URL;
...
export default function handler(req, res) {
  if (!SUELDOS_URL) {
    res.statusCode = 500;
    res.end(JSON.stringify({ error: 'VITE_SUELDOS_API_URL no configurada en Vercel' }));
    return;
  }

  const qs     = req.url.includes('?') ? req.url.slice(req.url.indexOf('?')) : '';
  const target = SUELDOS_URL + qs;
