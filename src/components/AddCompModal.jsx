...
import { DOCS, CUENTAS, CUENTA_LABEL, MOV_TYPES, TIPOS_MOVIMIENTO, SYM, fmt, uid, makeType } from "../lib/helpers";
...
export default function AddCompModal({ franchise, month, year, onClose, onAdd }) {
  ...
  const handleAdd = useCallback(() => {
    onAdd({ id: uid(), type, date, amount: totalConIVA, amountNeto: amount, ref, nota, month, year });
    onClose();
  }, [type, date, totalConIVA, amount, ref, nota, month, year, onAdd, onClose]);
  ...
  return (
    <Modal title={`Nuevo comprobante — ${franchise.name}`} subtitle={`${franchise.months ?? ""}${month !== undefined ? ["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"][month] : ""} ${year}`} onClose={onClose} width={520}>
      ...
      <Field label={`Monto neto (${franchise.currency})`}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span className="mono" style={{ color: "var(--muted)", fontSize: 12 }}>{SYM[franchise.currency]}</span>
      ...
      {showIVA && amount > 0 && (
        <div style={{ background: "rgba(251,191,36,.05)", border: "1px solid rgba(251,191,36,.15)", borderRadius: 6, padding: "10px 14px", marginBottom: 14, fontSize: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", color: "var(--muted)" }}><span>Neto</span><span className="mono">{fmt(amount, franchise.currency)}</span></div>
          <div style={{ display: "flex", justifyContent: "space-between", color: "var(--muted)" }}><span>IVA</span><span className="mono">{fmt(amount * TAX, franchise.currency)}</span></div>
          <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, marginTop: 4 }}><span>Total</span><span className="mono" style={{ color: "var(--accent)" }}>{fmt(totalConIVA, franchise.currency)}</span></div>
        </div>
      )}
