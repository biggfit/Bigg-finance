 ...
 * Excludes PAGO_PAUTA entries (they are not debit/credit movements).
 */
export function computeSaldo(frId, y, m, comps, saldoInicial) {
  const key = String(frId);
  let saldo = saldoInicial[key] ?? 0;
  const cutoff = `31/${String(m + 1).padStart(2,'0')}/${y}`;
  const frComps = (comps[key] ?? [])
    .filter(c => !SKIP_CC_TYPES.has(c.type) && cmpDate(c.date, cutoff) <= 0);
...
export function computeSaldoPrevMes(frId, y, m, comps, saldoInicial) {
  const pm = m - 1 < 0 ? 11 : m - 1;
  const py = m - 1 < 0 ? y - 1 : y;
  return computeSaldo(frId, py, pm, comps, saldoInicial);
}

export function buildCuentaCorriente(frId, comps, saldoInicial) {
  const key = String(frId);
  let saldo = saldoInicial[key] ?? 0;
  const lines = [{ type: "apertura", label: "Saldo de apertura", debit: 0, credit: 0, saldo, date: "31/12/2025" }];
  const frComps = (comps[key] ?? [])
    .filter(c => !SKIP_CC_TYPES.has(c.type))
    .sort((a, b) => cmpDate(a.date, b.date));
  for (const c of frComps) {
    const sign = COMP_TYPES[c.type]?.sign;
    if (sign === undefined) continue;
    const amount = c.amount ?? 0;
    if (sign === +1) { saldo += amount; lines.push({ ...c, debit: amount, credit: 0, saldo }); }
    else if (sign === -1) { saldo -= amount; lines.push({ ...c, debit: 0, credit: amount, saldo }); }
  }
