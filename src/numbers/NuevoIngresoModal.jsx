...
import { T } from "./theme";
import { CENTROS_COSTO, CUENTAS, CLIENTES_SEED } from "../data/numbersData";
...
const CUENTAS_INGRESO = CUENTAS.filter(c => c.tipo === "ingreso");
...
export default function NuevoIngresoModal({ onClose, onSave }) {
  ...
  const handleCliChange = (id) => {
    setCliId(id);
    const c = CLIENTES_SEED.find(x => x.id === id);
    if (!c) return;
  ...
  const handleSave = () => {
    const cli    = CLIENTES_SEED.find(c => c.id === cliId);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.5)", zIndex:400,
      ...
      <div className="fade" onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ background:"#1e3a5f", padding:"16px 24px",
          ...
          <div>
            <div style={{ fontSize:16, fontWeight:900, color:"#93c5fd" }}>Nueva Factura de Venta</div>
                ...
                <div key={l.id} style={{ display:"grid",
                  ...
                  <button onClick={() => delLinea(l.id)} disabled={lineas.length === 1}
                    style={{ background:"transparent", border:"none",
                      cursor: lineas.length > 1 ? "pointer" : "default",
                      color: lineas.length > 1 ? T.red : T.dim,
