...
// RegistrarCobroModal se movió a ./pagos/RegistrarCobroModal (reusado también por el reporte CxC por cliente).
...
function EditarCobroModal({ cobro, sociedad, cuentasSoc, cuentasContables = [], centros = [], onClose, onSaved }) {
  ...
  const cuentasOpts = cuentasSoc
    .filter(c => c.moneda === cobro.moneda)
...
function DetalleModal({ ingreso, cuentasBancarias = [], centrosCosto = [], onClose, onAgregarCobro, onAgregarRetencion, onEditar, onEditarCobro, onIrACaja, asPage = false }) {
  ...
  const aCobrar    = ingreso.importe - cobrado;
  const sobrecobro = aCobrar < -0.005;
  const lineas     = ingreso.lineas ?? [];
  ...
  const estadoCfg = ESTADO_INGRESO[ingreso.estado] ?? {};
  const estadoBg  = estadoCfg.bg ?? "#e5e7eb";
  ...
  return (
    <div
      ...
      <div
        ...
        {/* Header azul */}
        <div style={{ background:"#1e3a5f", padding:"13px 22px",
          ...(asPage ? {} : { flexShrink:0 }),
          display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontSize:15, fontWeight:900, color:"#93c5fd" }}>Ingreso</div>
            <div style={{ fontSize:11, color:"rgba(147,197,253,.4)", marginTop:1,
              fontFamily:"var(--mono)" }}>{ingreso.id}</div>
          </div>
          {asPage
            ? <button onClick={onClose} style={{ background:"transparent", border:"none",
        ...
        <div style={asPage ? {} : { flex:1, overflowY:"auto" }}>
          ...
          <div style={{ display:"flex", alignItems:"stretch", borderBottom:`1px solid ${T.cardBorder}`,
            background:"#f8f9fa" }}>
            {[
              { label:"Total Venta", value: fmtMoney(ingreso.importe, ingreso.moneda), color:"#1a1a1a", },
              { label:"Cobrado",     value: fmtMoney(cobrado,         ingreso.moneda), color: T.green,  },
              { label: sobrecobro ? "⚠ Sobrecobro" : "A Cobrar",
                value: fmtMoney(Math.abs(aCobrar), ingreso.moneda),
                color: sobrecobro ? T.orange : aCobrar > 0.005 ? T.red : T.green },
            ].map((item, i, arr) => (
          ...
          {/* ── COBROS (primero, header rojo Contagram) ── */}
          <div style={{ padding:"18px 24px 0" }}>
            <div style={{ background:"#c0392b", padding:"10px 16px", marginBottom:0,
              ...
              <span style={{ fontSize:13, fontWeight:800, color:"#fff", letterSpacing:".06em",
                textTransform:"uppercase" }}>Cobros</span>
            </div>
                    ...
                    : ingreso.pagosVinculados.map((p, pi) => (
                      <tr key={p.id} style={{ borderBottom:`1px solid ${T.cardBorder}`,
                        ...
                        <td style={{ padding:"8px 8px", textAlign:"center" }}>
                          <button onClick={() => onEditarCobro?.(p)} title="Editar cobro"
          ...
          <div style={{ padding:"16px 24px 14px", display:"grid", gridTemplateColumns:"1fr 1fr",
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
              {infoRow("Cliente", ingreso.cliente)}
              {infoRow("CUIT",    ingreso.cuit)}
              {infoRow("Moneda",  ingreso.moneda)}
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
              {infoRow("N° Comprobante", ingreso.nroComp)}
              {infoRow("Fecha emisión",  fmtDate(ingreso.fecha))}
              {infoRow("Vencimiento",    fmtDate(ingreso.vto), ingreso.estado === "vencido" ? T.red : undefined)}
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                ...
                <span style={{ display:"inline-block", padding:"2px 9px", borderRadius:12,
                  ...
                  {estadoCfg.label ?? ingreso.estado}
                </span>
              </div>
            </div>
          </div>
                  ...
                  : lineas.map((l, i) => (
                    <tr key={i} style={{ background:i%2===0?"#f2f3f5":"#e9eaec",
                      ...
                      <td style={{ padding:"9px 14px", fontSize:13, textAlign:"right",
                        fontFamily:"var(--mono)", color:"#1a1a1a" }}>{fmtMoney(Number(l.subtotal)||0, ingreso.moneda)}</td>
                      <td style={{ padding:"9px 14px", fontSize:13, textAlign:"right",
                        color:"#444" }}>{Number(l.ivaRate)||0}%</td>
                      <td style={{ padding:"9px 14px", fontSize:13, textAlign:"right",
                        fontFamily:"var(--mono)", fontWeight:700, color:"#111" }}>
                        {fmtMoney(Number(l.total_linea)||0, ingreso.moneda)}
                      </td>
                    </tr>
                  ))
                }
              ...
              <div style={{ flex:1, padding:"10px 4px 4px" }}>
                {ingreso.nota && (
                  <>
                    ...
                    <div style={{ fontSize:13, color:"#444", lineHeight:1.5 }}>{ingreso.nota}</div>
                  </>
                )}
              </div>
              <div style={{ background:"#606060", color:"#fff", minWidth:360,
                ...
                {netoNoGravado > 0 && (
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:13 }}>
                    ...
                    <span style={{ fontFamily:"var(--mono)" }}>{fmtMoney(netoNoGravado, ingreso.moneda)}</span>
                  </div>
                )}
                {netoGravado > 0 && (
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:13 }}>
                    <span>Importe Neto Gravado</span>
                    <span style={{ fontFamily:"var(--mono)" }}>{fmtMoney(netoGravado, ingreso.moneda)}</span>
                  </div>
                )}
                {ivaGroups.map(rate => {
                  ...
                  return (
                    <div key={rate} style={{ display:"flex", justifyContent:"space-between", fontSize:13 }}>
                      <span>IVA {rate}%</span>
                      <span style={{ fontFamily:"var(--mono)" }}>{fmtMoney(amt, ingreso.moneda)}</span>
                    </div>
                  );
                })}
                ...
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, fontWeight:700 }}>
                  ...
                  <span style={{ fontFamily:"var(--mono)" }}>{fmtMoney(ingreso.importe, ingreso.moneda)}</span>
                </div>
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, fontWeight:700 }}>
                  <span>Total Cobrado</span>
                  <span style={{ fontFamily:"var(--mono)" }}>{fmtMoney(cobrado, ingreso.moneda)}</span>
                </div>
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:14, fontWeight:800,
                  color: sobrecobro ? "#fca5a5" : aCobrar === 0 ? "#86efac" : "#fde68a" }}>
                  <span>{sobrecobro ? "⚠ Sobrecobro" : "Total a Cobrar"}</span>
                  <span style={{ fontFamily:"var(--mono)" }}>
                    {sobrecobro ? "−" : ""}{fmtMoney(Math.abs(aCobrar), ingreso.moneda)}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div style={{ padding:"12px 22px", borderTop:`1px solid ${T.cardBorder}`,
          ...
          <Btn variant="primary" onClick={() => { onClose(); onEditar(ingreso); }}>Editar</Btn>
        </div>
      </div>
    </div>
  );
...
function CtaCteModal({ cliente, documentos, onClose }) {
  const totalFacturado = documentos.reduce((s, d) => s + d.importe, 0);
  const totalCobrado   = documentos.reduce((s, d) => s + (d.importe - (d.saldoPendiente ?? d.importe)), 0);
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.5)", zIndex:500,
      ...
      <div className="fade" style={{ background:"#f8f9fa", borderRadius:12, width:820,
        ...
        <div style={{ background:"#1e3a5f", padding:"14px 22px", flexShrink:0,
          display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontSize:15, fontWeight:900, color:"#93c5fd" }}>
        ...
        <div style={{ background:"#fff", borderBottom:`1px solid ${T.cardBorder}`,
          ...
          {[
            ...
            { label:"Total Cobrado",   value: fmtMoney(totalCobrado),   color:T.green },
            { label:"Saldo Pendiente", value: fmtMoney(totalPendiente), color: totalPendiente>0 ? T.blue : T.green },
          ].map((item, i) => (
            <div key={item.label} style={{ flex:1, padding:"14px 20px",
            ...
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead>
                <tr style={{ background:T.tableHead }}>
                  {["Fecha","Vencimiento","N° Comp.","Cuenta","Importe","Cobrado","Pendiente","Estado"].map(h => (
                    <th key={h} style={{ padding:"9px 14px", fontSize:11, fontWeight:700,
                      color:T.tableHeadText, textAlign: ["Importe","Cobrado","Pendiente"].includes(h) ? "right" : "left",
              ...
              <tbody>
                {documentos.length === 0 ? (
                  ...
                  const cobrado   = d.importe - (d.saldoPendiente ?? d.importe);
...
export default function PantallaIngresos({ sociedad = "nako", subView = null, onSubViewChange, navPulse = 0, openDocId = null, onDocOpened, onIrACaja }) {
  ...
  // Cuentas bancarias de la sociedad activa (para medios de cobro)
  ...
  const cargarIngresos = async () => {
    ...
    setError(null);
    try {
      const [docs, pagos] = await Promise.all([fetchIngresos(sociedad), fetchPagosCobros(sociedad)]);
  ...
  useEffect(() => {
    cargarIngresos();
    fetchClientes().then(data => setClientes(Array.isArray(data) ? data : [])).catch(()=>{});
  ...
  // Cuando ingresos se recarga, sincronizar showDetalle con datos frescos
  useEffect(() => {
  ...
  const handleSave = async (ingreso) => {
    try {
      ...
      if (subView === "new-venta" && onSubViewChange) {
        ...
        const nuevoIngreso = {
          ...ingreso,
          id:             result?.id_comp ?? ingreso.id,
          importe:        ingreso.importe ?? 0,
          saldoPendiente: ingreso.importe ?? 0,
          pagosVinculados: [],
          estado:         "a_cobrar",
        };
        setShowDetalle(nuevoIngreso);
        if (ingreso._saveAndCobrar) {
          setShowCobro(nuevoIngreso);
        }
      }
  ...
  const totalesPorMoneda = useMemo(() => {
    ...
    return monedas.map(moneda => {
      ...
      return {
        ...
        cobrado: docs.reduce((s,e) => s + (e.pagosVinculados?.reduce((ps,p) => ps + (Number(p.monto)||0), 0) ?? 0), 0),
        aCobrar: docs.filter(e => e.estado === "a_cobrar").reduce((s,e) => s + (e.saldoPendiente ?? e.importe), 0),
  ...
  return (
    <div style={{ padding:"28px 32px", maxWidth:1200 }} className="fade">
      <PageHeader
        ...
        action={<Btn variant="accent" onClick={() => onSubViewChange?.("new-venta")}>+ Nuevo Ingreso</Btn>}
      />
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:24 }}>
        {totalesPorMoneda.map(t => (
          <div key={t.moneda} style={{ display:"flex", gap:8, alignItems:"stretch" }}>
            ...
            <CompactCard label="Cobrado"  value={fmtMoney(t.cobrado, t.moneda)} color={T.green} />
            <CompactCard label="A Cobrar" value={fmtMoney(t.aCobrar, t.moneda)} color={T.blue} />
                ...
                <tr key={e.id} style={{ borderBottom:`1px solid ${T.cardBorder}`,
                  ...
                  <td style={{ padding:"10px 14px" }}>
                    {[...new Set((e.pagosVinculados??[]).map(p=>p.cuenta_bancaria).filter(Boolean))].map(id => (
                      <span key={id} style={{ display:"inline-block", fontSize:11, background:"#dcfce7",
                        color:"#166534", borderRadius:6, padding:"2px 8px", fontWeight:600, marginRight:3 }}>
                        {resolveCB(id)}
                      </span>
                    ))}
