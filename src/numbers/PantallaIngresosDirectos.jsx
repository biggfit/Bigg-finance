...
import {
  fetchIngresosDirectos, deleteIngresoDirecto, appendIngresoDirecto, updateIngresoDirecto,
...
function FormNuevoIngreso({ sociedad, cuentasBancarias, cuentas, centrosCosto, proveedores, onClose, onSaved, editIngreso = null }) {
  ...
  const [rows, setRows]     = useState(() => {
    if (!editIngreso) return [newRow()];
    const prov = proveedores.find(p => p.nombre === editIngreso.proveedor);
    return [{
      _id:             Date.now(),
      fecha:           editIngreso.fecha,
      proveedorId:     prov?.id ?? "",
      cuenta_contable: editIngreso.cuenta_contable,
      cc:              editIngreso.cc,
      subtotal:        String(editIngreso.subtotal),
      ivaRate:         String(editIngreso.ivaRate),
      medioPago:       editIngreso.cuentaBancaria,
      nota:            editIngreso.nota ?? "",
    }];
  });
  ...
  const handleGuardar = async () => {
    ...
    try {
      if (editIngreso) {
        ...
        await updateIngresoDirecto(editIngreso._movId, {
          fecha:              r.fecha,
          cuenta_contable:    r.cuenta_contable,
          cuenta_contable_id: cuentasIngreso.find(c => c.nombre === r.cuenta_contable)?.id ?? "",
          cc:                 r.cc,
          moneda:             getMoneda(r.medioPago) || editIngreso.moneda,
  ...
  return (
    <div className="fade" style={{ padding:"28px 32px" }}>
      {/* Header */}
      <div style={{ background:"#064e3b", borderRadius:T.radius, padding:"14px 24px",
        display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <div>
          <div style={{ fontSize:16, fontWeight:900, color:"#6ee7b7" }}>
            {editIngreso ? "Editar Ingreso" : "Ingresos Directos"}
          </div>
          <div style={{ fontSize:11, color:"rgba(110,231,183,.55)", marginTop:3 }}>
            Cobrados en el momento (sin factura) · afectan P&amp;L y Tesorería · La moneda se toma de la caja/banco seleccionado
          </div>
        </div>
        <button onClick={onClose} style={{ background:"transparent", border:"none",
          color:"rgba(110,231,183,.7)", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:T.font }}>
          ← Volver
        </button>
      </div>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", tableLayout:"fixed" }}>
            ...
            <thead>
              <tr style={{ background:"#064e3b" }}>
                {[
                  ["Fecha","left"], ["Contraparte / Concepto","left"],
                  ["Cuenta Contable","left"], ["Centro de Costo","left"],
                  ["Subtotal","right"], ["IVA","center"],
                  ["Total","right"], ["Caja / Banco","left"], ["Nota","left"], ["","left"],
                ].map(([h, align]) => (
                  <th key={h} style={{ padding:"10px 10px", fontSize:11, fontWeight:700,
                    color:"#6ee7b7", textAlign:align, letterSpacing:".06em",
                  ...
                  <tr key={r._id} style={{ borderBottom:`1px solid ${T.cardBorder}`,
                    ...
                    <td style={{ padding:"6px 8px" }}>
                      <select value={r.cuenta_contable}
                        ...
                        {cuentasIngreso.map(c => <option key={c.id} value={c.nombre}>{c.nombre}</option>)}
                      </select>
                    </td>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:14 }}>
        <div style={{ fontSize:12, color:T.muted }}>
          {validRows.length > 0
            ? <span style={{ color:"#16a34a", fontWeight:700 }}>
                ✓ {validRows.length} ingreso{validRows.length !== 1 ? "s" : ""} listo{validRows.length !== 1 ? "s" : ""} para guardar
              </span>
            : <span>Completá al menos 1 fila (cuenta contable + centro de costo + caja/banco + subtotal)</span>
          }
        </div>
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={handleGuardar} disabled={!canSave || saving}
            style={{ background: canSave && !saving ? "#059669" : "#9ca3af", border:"none",
            ...
            {saving ? "Guardando…" : editIngreso
              ? "Guardar cambios ✓"
              : `Registrar ${validRows.length > 0 ? validRows.length : ""} Ingreso${validRows.length !== 1 ? "s" : ""} ✓`}
          </button>
        </div>
      </div>
    </div>
...
export default function PantallaIngresosDirectos({ sociedad = "nako", subView = null, onSubViewChange, navPulse = 0 }) {
  ...
  // Tocar un ítem del sidebar cierra el ingreso en edición y vuelve a la lista.
  useEffect(() => { setEditing(null); }, [navPulse]); // eslint-disable-line
  ...
  const cargar = async () => {
    ...
    setError(null);
    try {
      setIngresos(await fetchIngresosDirectos(sociedad));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  if (subView === "new-ingreso" || editing) {
    return (
      <FormNuevoIngreso
        sociedad={sociedad}
        ...
        editIngreso={editing}
        onClose={() => { setEditing(null); onSubViewChange?.("ingresos-directos"); }}
        onSaved={async () => { setEditing(null); onSubViewChange?.("ingresos-directos"); await cargar(); }}
      />
    );
  }
  ...
  return (
    <div style={{ padding:"28px 32px" }} className="fade">
      <PageHeader
        ...
        action={<Btn variant="accent" onClick={() => onSubViewChange?.("new-ingreso")}>+ Nuevo Ingreso</Btn>}
      />
      ...
      <div style={{ background:T.card, border:`1px solid ${T.cardBorder}`, borderRadius:T.radius,
        ...
        <input
          ...
          placeholder="Buscar por cuenta contable, CC, caja, nota…"
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", minWidth:700 }}>
          <thead>
            <tr style={{ background:T.tableHead }}>
              {["Fecha","Contraparte","Cuenta Contable","Centro de Costo","Monto","Caja / Banco","Registró",""].map(h => (
          ...
          <tbody>
            {rows.length === 0
              ? <tr><td colSpan={8} style={{ padding:32, textAlign:"center", color:T.dim, fontSize:13 }}>
                  {ingresos.length === 0 ? "Sin ingresos registrados" : "Sin resultados"}
                </td></tr>
              : rows.map((g, i) => (
                <tr key={g.id} style={{ borderBottom:`1px solid ${T.cardBorder}`,
                  ...
                  <td style={{ padding:"9px 14px", textAlign:"right", fontFamily:"var(--mono)",
                    fontSize:12, fontWeight:700, color:"#16a34a", whiteSpace:"nowrap" }}>
