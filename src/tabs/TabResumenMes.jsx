...
import { useFrData } from "./TabSaldos";
...
const TabResumenMes = memo(function TabResumenMes({ allFranchises, month, year, onNavigate }) {
  const frData = useFrData(allFranchises, month, year);

  const tarjetas = useMemo(() => {
    const CUR_ORDER = ["ARS","USD","EUR"];
    const currs = CUR_ORDER.filter(c => allFranchises.some(f => f.currency === c));
    return currs.map(cur => {
      const rows = frData.filter(d => d.fr.currency === cur);
      if (!rows.length) return null;
      const sum = (fn) => rows.reduce((a, d) => a + fn(d), 0);
      return {
        cur,
        totSP:          sum(d => d.sp),
        totSA:          sum(d => d.sa),
        ...
        nDeben:         rows.filter(d => d.sa >  0.01).length,
        cobrarReal:     rows.filter(d => d.sa < -0.01 && d.pautaPendiente < 0.01),
        pautaPendRows:  rows.filter(d => d.pautaPendiente > 0.01),
        totPautaPend:   rows.filter(d => d.pautaPendiente > 0.01).reduce((a, d) => a + d.pautaPendiente, 0),
        totDeben:       rows.filter(d => d.sa >  0.01).reduce((a,d) => a + d.sa, 0),
      };
    }).filter(Boolean);
  }, [frData, allFranchises]);
